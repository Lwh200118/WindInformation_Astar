...
def LineOfSight(s, e, obmap):
    ...
    err = dx + dy
    max_iterations = 100000  # 设置一个足够大的迭代次数上限
    iteration = 0
...
class AStarPlanner:
    def __init__(self, ox, oy, resolution, rr, data_dir):
        ...
        self.start_time = 3600  # 起始时间
        self.time_interval = 1.0   # 时间间隔
        self.U_t = 8  # 固定速度值 (m/s)
        self.minx = -100  # 修改为-100
        self.miny = 0     # 修改为0
        self.maxx = 500   # 修改为500
        self.maxy = 400   # 修改为400
        self.calc_obstacle_map(ox, oy)  # 初始化障碍物地图
    ...
    def load_data(self, time):
        rounded_time = round(time)  # 四舍五入到最近的整数秒
        folder_name = f"{rounded_time:.1f}"  # 保留一位小数的字符串
        filename = os.path.join(self.data_dir, folder_name, "U_0_mySurfaces_60.dat")
        print(f"Trying to load data from: {filename}")  # 添加调试信息
        if os.path.exists(filename):
            self.current_data = read_data(filename, minx=self.minx, maxx=self.maxx, miny=self.miny, maxy=self.maxy)
        else:
    ...
    def expand_grid(self, current, ngoal, open_set, closed_set, c_id, nstart):
        for i, _ in enumerate(self.motion):
            ...
            if distance_sum >= 1.25* initial_distance_to_goal:
                continue
            ...
            # 更新风场数据，使用四舍五入的时间点，并保留一位小数
            self.load_data(round(node.time))
...
def main():
    ...
    plt.title("Astar_PM_V8_Instant_City_More_Nod_1.0s")
    sx, sy, gx, gy = 0, 200, 320, 200
    grid_size, robot_radius = 1.25, 5.0
    width, height = 2600, 2600  # 修改为2600的长宽
    ...
    output_filename = f'New_optimized_UW_PM_path_Instant_City_{sx}_{sy}_to_{gx}_{gy}_V8UTT_More_Mod_1_0.dat'
