...
class AStarPlanner:
    ...
    def __init__(self, ox, oy, resolution, rr, data):
        ...
        self.interpolation_cache_w = {}  # 新增缓存字典
        # 设置日志记录器
    ...
    def planning(self, sx, sy, gx, gy):
        ...
        open_set = []
        closed_set = {}  # 修改为字典
        heapq.heappush(open_set, (nstart.cost, self.calc_grid_index(nstart), nstart))

        iteration = 0  # 添加一个计数器变量

        while open_set:
            _, c_id, current = heapq.heappop(open_set)
            ...
            if (current.x, current.y) == (ngoal.x, ngoal.y):
                print("Find goal!")
                ngoal.parent_index = current.parent_index
                ngoal.cost = current.cost
                break

            closed_set[c_id] = current  # 将节点添加到 closed_set 字典中

            self.expand_grid(current, ngoal, open_set, closed_set, c_id)

        pathx, pathy = self.calc_final_path(ngoal, closed_set)
        return pathx, pathy

    def expand_grid(self, current, ngoal, open_set, closed_set, c_id):
        for i, _ in enumerate(self.motion):
            ...
            WindEffect = self.calculate_cost_Vi(dx, dy, u, v, w)
            
            # 更新节点成本
            node.cost = current.cost + WindEffect  # 更新总成本     
            # 输出调试信息
            print(f'当前节点: x={current.x * self.resolution}, y={current.y * self.resolution}；计算成本: dx={dx}, dy={dy}, u={u}, v={v}, w={w}, cost={node.cost}, g={current.g},h={current.h}')
            print(f'新节点: x={node.x * self.resolution},y={node.y * self.resolution}')       
            ...
            if n_id not in [i[1] for i in open_set]:
                heapq.heappush(open_set, (node.cost, n_id, node))
            else:
                for open_node in open_set:
                    if open_node[1] == n_id and open_node[2].cost > node.cost:
    ...
    def calculate_cost_Vi(self, dx, dy, u, v, w, Cd=1, S=1):
        # 常量定义
...
def main():
    filename = r"E:\UAV\研究计划\WindFieldInfo\U10_H30.dat"
    print(__file__ + '  start!')
    plt.title("Astar_PM")
    sx, sy, gx, gy =  60,80,160,80
    ...
    data = read_data(filename)
    a_star = AStarPlanner(ox, oy, grid_size, robot_radius, data)  # 注意这里传入了data
    ...
    output_filename = f'New_optimized_UW_Vi_path_{sx}_{sy}_to_{gx}_{gy}_V16.dat'
