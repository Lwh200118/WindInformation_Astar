...
class AStarPlanner:
    ...
    def expand_grid(self, current, ngoal, open_set, closed_set, c_id):
        for i, _ in enumerate(self.motion):
            ...
            u = self.interpolate_v(new_x, new_y)
            v = self.interpolate_u(new_x, new_y)
            ...
            # 输出调试信息
            print(f'当前节点: x={current.x * self.resolution}, y={current.y * self.resolution}；计算成本: dx={dx}, dy={dy}, u={u}, v={v}, w={w}, cost={node.cost}, g={node.g},h={node.h}')
...
def main():
    ...
    plt.title("Astar_DO_V8_PM")
    sx, sy, gx, gy =  100,40,100,140
    ...
    output_filename = f'New_optimized_UW_PM_path_{sx}_{sy}_to_{gx}_{gy}_V802.dat'
