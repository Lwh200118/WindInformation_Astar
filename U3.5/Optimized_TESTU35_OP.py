...
class AStarPlanner:
    ...
    def calc_heuristic(n1, n2, R=100, K=0.5):  # n1: ngoal，n2: open_set[o]
        ...
        Dy = abs(n1.y - n2.y)
        D = 1.25  # 曼哈顿距离系数
        D2 = 0.8 * math.sqrt(2)  # 切比雪夫距离系数

        h = D * ((Dx + Dy) - min(Dx, Dy)) + D2 * min(Dx, Dy)
    ...
    def calculate_cost(self, dx, dy, u, v, w, Cd=1, S=1):
       ...
       # 使用给定的公式计算成本
        cost = ((0.5 * rho * Cd * S * (wind_effect_u + wind_effect_v + abs(w**3)) + P_i) + 1650 ) * D / U_tt
        # + m * g * D
        return cost
...
def main():
    filename = r"E:\UAV\研究计划\WindFieldInfo\UMean_H75_h60_V8.dat"
    print(__file__ + '  start!')
    plt.title("Astar_PM_V8_h60_V8_W77")
    sx, sy, gx, gy = 50,100,150,100
    ...
    output_filename = f'New_optimized_UW_PMU35_path_{sx}_{sy}_to_{gx}_{gy}_V8UTT_h60_V_W72.dat'
