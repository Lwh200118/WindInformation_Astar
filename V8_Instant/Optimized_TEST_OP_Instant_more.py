...
class AStarPlanner:
    def __init__(self, ox, oy, resolution, rr, data_dir):
        ...
        self.start_time = 5000  # 起始时间
        self.time_interval = 5  # 时间间隔
    ...
    def planning(self, sx, sy, gx, gy):
        ...
        while True:
            ...
            # 计算当前节点与起点的距离，并依据飞行速度计算时间点
            distance = math.hypot(current.x - nstart.x, current.y - nstart.y) * self.resolution
            time_elapsed = distance / self.U_t
            current_time = self.start_time + int(time_elapsed / self.time_interval) * self.time_interval

            # 加载当前时间点的风场数据
            self.load_data(current_time)
...
def main():
    data_dir = r"E:\UAV\研究计划\WindFieldInfo\Instant_field\U_0"
    print(__file__ + '  start!')
    plt.title("Astar_PM_V8_Instant_More")
    sx, sy, gx, gy = 50,100,230,100
    ...
    output_filename = f'New_optimized_UW_PM_path_Instant_{sx}_{sy}_to_{gx}_{gy}_V8UTT_More.dat'
